...
const Modificar_Ahorro = gql `
    mutation ModificarAhorro($id: ID, $input: AhorroInput) {
      modificarAhorro(id: $id, input: $input) {
        id
        usuarioId
        clienteId
        sucursalId
...
const ModificarAhorro = () => {

    const [mensaje, guardarMensaje] = useState(null)

    const [ modificarAhorro ] = useMutation(Modificar_Ahorro)
    ...
    const formik = useFormik({
        ...
        onSubmit: async valores => {
            ...
            let { id, clienteId, sucursalId, monto, plazo, moneda } = valores

            monto = parseFloat(monto)

            try {

                const { data } = await modificarAhorro ({
                    variables: {
                        id,
                        input: {
                            clienteId,
    ...
    return (
        <>
            <LayoutAhorro>
                ...
                <h1 className="text-2xl text-gray-200 font-light">Modificar Ahorro</h1>
...
export default ModificarAhorro;