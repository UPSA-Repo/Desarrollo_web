...
const AskingAccountOrigin = () => {
    const [account, setAccount] = useState([])

    const listContext = useContext(ListContext)
    const {clientOrigin,AddAccountOrigin} = listContext

    const [loadAccount, {called, loading, data}] = useLazyQuery(GET_ACCOUNTS,{
        variables: {
            clientId: clientOrigin.id
        }
    })

    useEffect(() => {
        AddAccountOrigin(account)
    },[account])

    const selectAccountOrigin = (account) => {
        setAccount(account)
    }
    ...
    return (
        <>
            <p className="mt-4 pl-4 my-2 bg-white border-l-8 border-green-500 text-gray-700 p-1 text-sm font-bold uppercase">Select a Account Origin</p>
            <Select
                className="mt-3 font-bold text-center"
                options={data.getAccountsClient}
                onChange={ (option) => selectAccountOrigin(option)}
...
export default AskingAccountOrigin